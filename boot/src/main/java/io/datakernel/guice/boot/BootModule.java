...
public final class BootModule extends AbstractModule {
	...
	private Map<Key<?>, List<KeyInPool>> mapKeys;
	...
	private final class NioWorkerScope implements Scope, WorkerThreadsPool {
				...
				public T get() {
					...
					if (current == null && !scopedObjects.containsKey(key)) {
						...
						if (!mapKeys.containsKey(key))
							mapKeys.put(key, new ArrayList<KeyInPool>());
						mapKeys.get(key).add(new KeyInPool(key, currentPool));
					}
					return current;
				}
			};
		...
		private Map<Key<?>, List<KeyInPool>> getMapKeys() {
			if (mapKeys == null) {
