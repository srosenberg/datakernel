...
public class ServerSetupExample {
	...
	public static void main(String[] args) throws IOException {
		...
		for (ServerInfo server : bootstrap) {
			Path serverStorage = Paths.get("./test_storage/server_storage_" + server.getServerId());
			Path tmpStorage = Paths.get("./test_storage/tmp_" + server.getServerId());
			HashFsServer.build(eventloop, newCachedThreadPool(), serverStorage, tmpStorage, server, bootstrap)
					.build()
