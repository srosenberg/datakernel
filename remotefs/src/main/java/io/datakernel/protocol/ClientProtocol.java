...
public class ClientProtocol {
	...
	// connect callbacks
	protected ConnectCallback uploadConnectCallback(final InetSocketAddress address, final String fileName,
			...
			public void onConnect(SocketChannel channel) {
				SocketConnection connection = createConnection(channel)
						...
						.addHandler(Err.class, new MessagingHandler<Err, FsCommand>() {
							...
							@Override
							public void onException(Exception e) {
								logger.trace("Stream exception while uploading file {}", fileName);
								callback.onException(e);
							}
						});
				connection.register();
			...
			public void onConnect(SocketChannel channel) {
				SocketConnection connection = createConnection(channel)
						.addStarter(new MessagingStarter<FsCommand>() {
							@Override
							public void onStart(Messaging<FsCommand> messaging) {
								logger.info("Send commit command for {} with status {}", fileName, success);
								messaging.sendMessage(new FsCommands.Commit(fileName, success));
							}
						})
						.addHandler(FsResponses.Ok.class, new MessagingHandler<FsResponses.Ok, FsCommand>() {
							@Override
							public void onMessage(FsResponses.Ok item, Messaging<FsCommand> messaging) {
								logger.info("Received ok for commit command {} with status {}", fileName, success);
								messaging.shutdown();
								if (success) {
									callback.onComplete();
								} else {
