...
public class AsyncSslSocketTest {
	...
	public void init() throws Exception {
		...
		SSLContext sslContect = createSslContext("TLSv1.2", keyManagers, trustManagers, new SecureRandom());

		SSLEngine serverSSLEngine = sslContect.createSSLEngine();
		serverSSLEngine.setUseClientMode(false);
		serverSocketStub = new AsyncTcpSocketStub("server");
		...
		SSLEngine clientSSLEngine = sslContect.createSSLEngine();
		clientSSLEngine.setUseClientMode(true);
		clientSocketStub = new AsyncTcpSocketStub("client");
...
//	@Ignore
	...
	@Test
	public void returnsSSLExceptionWhenOtherSideWasClosedWithoutSpecifiedHandshakeMessage() {
		context.checking(new Expectations() {{
			...
			ignoring(clientEventHandler).onRegistered();
			ignoring(serverEventHandler).onRegistered();
			ignoring(serverEventHandler).onClosedWithError(with(any(Exception.class)));
		}});
		...
		eventloop.post(new Runnable() {
			...
			public void run() {
				...
				eventloop.postLater(new Runnable() {
					@Override
					public void run() {
						// write endOfStream directly to stub socket, without informing serverSslSocket
						serverSocketStub.writeEndOfStream();
					}
				});
			}
		});
	...
	@Test
	public void returnsEndOfStreamWhenOtherSideProperlyClosedSSLConnection() {
		context.checking(new Expectations() {{
			// check first messages
			oneOf(serverEventHandler).onRead(with(bytebufOfMessage("Hello")));
			oneOf(clientEventHandler).onRead(with(bytebufOfMessage("World")));
		...
		eventloop.post(new Runnable() {
			...
			public void run() {
				...
				clientSslSocket.write(createByteBufFromString("Hello"));
				serverSslSocket.write(createByteBufFromString("World"));
				serverSslSocket.writeEndOfStream();
	...
	public static final class AsyncTcpSocketStub implements AsyncTcpSocket {
		...
		public AsyncTcpSocketStub(String desc) {
			this.desc = desc;
		}
