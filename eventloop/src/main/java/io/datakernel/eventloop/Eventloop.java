...
/**
 * It is internal class for asynchronous programming. An asynchronous
 * programming means that a unit of work runs separately from the main thread
 * and notifies the calling thread of its status (e.g completion, progress or
 * failure). Eventloop represents infinite loop with only one blocking
 * operation selector.select() which selects a set of keys whose corresponding
 * channels are ready for I/O operations. With these keys and queues with
 * tasks, which was added to Eventloop from the outside, it begins asynchronous
 * executing from one thread it in method run() which is overridden because it
 * is implementation of {@link Runnable}. Working of this eventloop will be
 * ended, when it has not selected keys and its queues with tasks are empty.
 */
